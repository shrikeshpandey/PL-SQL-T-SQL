...
||   -----------------------------------------------------------------------------
||   Luis Fuentes     7/04/2018    Initial Creation
...
USE [Clerk]
GO
/****** Object:  StoredProcedure [dbo].[T_DM_FINANCE_DATA_PART_2]    Script Date: 10/10/2018 10:35:25 AM ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
-- Author:		<Luis Enrique Fuentes>
-- Create date: <6/27/2018>
-- Description:	<This sp loads data into [dbo].[DASHBOARD_OMS]>
-- =============================================
ALTER PROCEDURE [dbo].[T_DM_FINANCE_DATA_PART_2]
...
BEGIN TRANSACTION;
	--Get last day loaded
	--DECLARE @lastDate AS DATETIME = CONVERT(VARCHAR,CAST((SELECT MAX([PERIOD]) FROM [Clerk].[dbo].[DASHBOARD_FINANCE_DATA] WHERE [SOURCE_TYPE] = 'FINANCE_DATA_PART_2') AS DATE));
	DECLARE @lastDate AS DATETIME = DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE())-1, 0)
	
	--Delete from table
	DELETE [Clerk].[dbo].[DASHBOARD_FINANCE_DATA]
	WHERE [PERIOD] = @lastDate AND [SOURCE_TYPE] = 'FINANCE_DATA_PART_2';
...
FROM (
select
'FINANCE_DATA_PART_2' AS SOURCE_TYPE,
...
--END 10/5/2018
 from
(select g.group_name, geos.geo from
dbo.geo_groups g
inner join dbo.geos on geos.geo_group_id = g.id
) gg
--BEGING 10/5/2018
INNER JOIN
--END 10/5/2018
...
'select
 to_date(''01-''||gl.f_acctperiod, ''dd-mon-yy'') as Period,
''GE'' as "Region",
case when gl.f_GLCC_GEO in (''275'', ''258'', ''225'', ''279'') then ''GE CORE''
     else ''GE OTHER''
end as DIVISION,
  gl.f_GLCC_GEO as Geo,
 PA.Description as "Geo Desc",
PA.Service_Area_Owner as "Director",
gl.f_glcc_acct,
gl.F_GLCC_CC,
acct.lvl2_descr,
acct.lvl4_descr,
...
left join DW.T_PFIN_CMPC_PA_REPORTING PA2 on gl.F_GLCC_CC = PA2.GEOGRAPHY
...
'Select
    ...
    (Sum(case when PT.f_hr_Dept_id in (''5363'', ''5457'') then PT.F_CCSI_WORK_HRS + PT.F_OT_HRS else 0 end))/8 as WD_SA_Days,
...
Where PT.F_DUR >= (select trunc(last_day(add_months(sysdate,-2))+1) FROM dual) --to_date(''01/01/2017'', ''mm/dd/yyyy'')
    ...
    and PT.f_hr_Dept_id in ( ''5364'', ''5363'', ''5300'', ''5457'')
group by
...
'Select
     ...
     sum( case when WT.department_code in ( ''5363'', ''5457'') then WT.Billable_Hours else 0 end)/8 as FG_SA_Days,
...
WHERE
   ...
   and WT.department_code in( ''5364'', ''5363'', ''5300'', ''5457'')
group by
   HR.F_RESOURCE_DISTRICT_ID,
   DT.CLNDR_MO_START_DT '
) ) FS
